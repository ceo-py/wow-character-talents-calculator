...
function onEdit(e) {
    ...
    if (sheetName === "Talents" && buttonscells.includes(a1)) {
        ...
        poinst_left_to_spend = GetCellValue("AC2");
        // CreateCell('G7', 5, poinst_left_to_spend);
        // CreateCell('I7', 5, poinst_left_to_spend);
        starlight = GetCellValue("G7");
        genesis = GetCellValue("I7");
        moonglow = GetCellValue("E11");
        nature_majesty = GetCellValue("G11");
        improved_moonfire = GetCellValue("K11");
        ...
        tier_one_and_two_result =
            starlight + genesis + moonglow + nature_majesty + improved_moonfire;
        // var test123 = GetCellValue('B18');
        if (starlight + genesis < 5) {
            buttonscells.forEach((cell) => {
                if (cell !== "I7" && cell !== "G7") {
                    ResetCell(cell);
                }
            });
    ...
    if (sheetName === "Talents" && buttons_resto.includes(a1)) {
        ButtonReset("AA11", "Talents!");
        ButtonReset("AA10", "Talents!");
        naturalice = GetCellValue("Q11");
        subtlety = GetCellValue("S11");
        nature_shape_shifter = GetCellValue("U11");
        intensity = GetCellValue("Q15");
        // var clear_casting = GetCellValue('S15');
        master_shape_shifte = GetCellValue("U15");
        // var tranquil_spirit = GetCellValue('S19');
        imp_rej = GetCellValue("U19");
        nature_switfness = GetCellValue("Q23");
        gift_of_nature = GetCellValue("S23");
        // var imp_tranq = GetCellValue('W23');
        emp_touch = GetCellValue("Q27");
        // var natures_bounty = GetCellValue('U27');
        // var living_spirit = GetCellValue('Q31');
        swiftmend = GetCellValue("S31");
        // var nature_perfection = GetCellValue('U31');
        emp_rej = GetCellValue("S35");
        // var living_seed = GetCellValue('U35');
        revitalize = GetCellValue("Q39");
        tree_of_life = GetCellValue("S39");
        imp_tree_of_life = GetCellValue("U39");
        // var imp_barkskin = GetCellValue('Q43');
        // var gote = GetCellValue('U43');
        wild_growth = GetCellValue("S47");
        mark_of_wild = GetCellValue("Q7");
        nature_focus = GetCellValue("S7");
        furor = GetCellValue("U7");
        tier_first_row = mark_of_wild + nature_focus + furor;
        tier_second_row =
            tier_first_row + naturalice + subtlety + nature_shape_shifter;
    ...
    if (
        ...
        buttons_four_peace.includes(a1) &&
    ...
    if (sheetName === "Calculator" && buttons_two_peace.includes(a1)) {
        ...
        buttons_two_peace.forEach((cell) => {
